...
$(document).ready(function(){
	   $('.t-datepicker').tDatePicker({
	     autoClose:true,
	     dateCheckIn:$("#e_startdate").val(),
	     dateCheckOut:$("#e_enddate").val(),
...
$(function(){
	...
	localStorage.imgFileCnt = "${fn:length(eventImageList)}" * 1 + 1;
	localStorage.totSize = 0;
	
	for(var i=1; i<="${fn:length(eventImageList)}"; i++) {
		var sizeId = "size" + i;
		localStorage.totSize = localStorage.totSize * 1 + document.getElementById(sizeId).childNodes[0].nodeValue * 1;
	}
		...
		if(localStorage.imgFileCnt > 10) {
			...
			if(imgFileExtend=="jpg" || imgFileExtend=="png" || imgFileExtend=="gif" || imgFileExtend=="jpeg" || imgFileExtend=="JPG" || imgFileExtend=="PNG" || imgFileExtend=="GIF" || imgFileExtend=="JPEG") {
				if(size <= 20971520 && localStorage.totSize <= 20971520) {
					...
					$("<tr id='"+id+"'><td><img src='#' id='preImg' class='preview'></td><td>"+imgFile+"</td><td id='size"+id+"'>"+size+"</td><td class='file_remove'><button type='button' class='btn btn-outline-secondary' onclick='file_remove(\""+id+"\", \""+localStorage.imgFileCnt+"\")'><i class='fas fa-trash-alt'></i></button></td></tr>").insertAfter("#fileTr");
...
$(document).ready(function(){
	$("#e_region option").each(function(){
		if($(this).val()=="${eventVO.e_region}"){
			$(this).attr("selected", "selected");
		}
	});
	
	var tag = "${eventVO.e_tag}".split(",");
	
	for(var i=0; i<tag.length; i++) {
		$("input:checkbox[value="+tag[i]+"]").iCheck('check');
	}
	
});

function deleteEvent(e_idx) {
	location.href="/deleteEvent.do?e_idx="+e_idx;
}

function existFileRemove(e_idx, e_realpath, e_server, e_img, e_main) {
	if(confirm("정말 파일을 삭제하시겠습니까?(기존 이미지 파일로 복구가 불가능합니다.)")){
		$.ajax({
			type:"GET",
			url:"/existFileRemove.do",
			data:"e_idx="+e_idx+"&e_realpath="+e_realpath+"&e_server="+e_server+"&e_img="+e_img,
			success: function(msg) {
				exists_file_remove(e_img, e_main);
			}
		});
	}
}

function modifyEvent(frm) {
	frm.action="/modifyEvent.do";
	frm.submit();
}
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">이벤트 관리</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            ...
            <div class="card card-warning">
				...
				<div class="card-body">
						...
						<div class="form-group">
							<label>제목</label>
							<input type="text" class="form-control" placeholder="이벤트명" name="e_name" value="${eventVO.e_name}">
						</div>
						<!-- textarea -->
						<div class="form-group">
							<label>내용</label>
							<textarea class="form-control" rows="3" placeholder="이벤트 내용" name="e_content">${eventVO.e_content}</textarea>
						</div>
						
						<!-- input states -->
						<fmt:formatDate value="${eventVO.e_startdate}" pattern="yyyy-MM-dd" var="e_startdate"/>
                        <fmt:formatDate value="${eventVO.e_enddate}" pattern="yyyy-MM-dd" var="e_enddate"/>
						<div class="form-group has-success">
	                        ...
	                        <input id="e_startdate" type="hidden" class="form-control" name="e_startdate" value="${e_startdate}" required data-eye > 
                            <input id="e_enddate" type="hidden" class="form-control" name="e_enddate" value="${e_enddate}" required data-eye>
						...
						<div class="주소">
							...
							<input id="autocomplete" type="text" class="form-control" name="e_address" value="${eventVO.e_address}">
                            <input class="field" id="lat" type="hidden" class="form-control" name="lat" value="${eventVO.lat}"/>
                            <input class="field" id="lng" type="hidden" class="form-control" name="lng" value="${eventVO.lng}"/>
                            <input type="hidden" class="form-control" name="e_idx" value="${eventVO.e_idx}"/>
						</div>
						...
						<div class="form-group">
							...
							<div class="custom-file">
								<input type="file" class="custom-file-input userFile" id="e_img${fn:length(eventImageList) + 1}">
								<label id="file-label" class="custom-file-label" for="e_img${fn:length(eventImageList) + 1}">이미지 업로드</label>
							</div>
						</div>
						...
						<table class="table table-bordered table-hover">
							...
							<c:forEach var="list" items="${eventImageList}">
							<tr id="${list.e_img}">
								<td><img src="${list.e_img}" id="pre" class='preview'></td>
								<td>${list.e_img}</td>
								<td id="size${list.e_main}">${list.e_size}</td>
								<td class='file_remove'>
									<button type='button' class='btn btn-outline-secondary' onclick="existFileRemove('${list.e_idx}', '${list.e_realpath}', '${list.e_server}', '${list.e_img}', '${list.e_main}')">
				...
				<div class="card-footer">
					<button type="button" class="btn btn-outline-secondary" id="e_modify_btn" onclick="modifyEvent(this.form)">수정</button>
					<button type="button" class="btn btn-outline-secondary" id="e_del_btn" onclick="deleteEvent('${eventVO.e_idx}')">삭제</button>
				</div>
				</form>
				</div>
				<!-- /.card -->
